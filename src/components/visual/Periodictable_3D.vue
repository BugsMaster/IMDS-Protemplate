<template>
    <div class="chart-periodictable">
		...
		<div id="menu">
			...
			<button @click="$router.push({ name: 'visualization' })">Visual</button>
		</div>
    </div>
</template>
    ...
    methods: {
        initPage(){
            ...
            let table = [
				...
				"漆黑", "BugsMaster", "(300)", 20, 9,
				"小T", "BugsMaster", "(Klax)", 20, 10
			];
			...
			function init() {
				...
				// camera = new THREE.PerspectiveCamera( 40, that.options.width / that.options.height, 1, 10000 );
				camera = new THREE.PerspectiveCamera( 40, that.options.width / that.options.height, 1, 10000 );
				...
				renderer.setSize( that.options.width, that.options.height );
				...
				let buttonSphere = document.getElementById( 'sphere' );
				...
				let buttonHelix = document.getElementById( 'helix' );
			...
			function onWindowResize() {
				...
				camera.aspect = that.options.width / that.options.height;
